...
input_file="/home/justind/oe75/csp1-3/data/vdb_dump/mh_1_pair.csv"
...
while IFS= read -r line && ([[ -z $num_rows ]] || [[ $count -lt $num_rows ]]); do
	script_path="${script_output_dir}/b8_megahit_assemble_paired_${line}.sh" 

#Put script below, using double backslash for continuing line
cat <<EOF > ${script_path}
#!/bin/bash

#SBATCH --job-name=megahit_assemble_paired_${line}
