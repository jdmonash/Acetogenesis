...
input_file="/home/justind/oe75/csp1-3/data/vdb_dump/megahit_test_single.csv"
...
while IFS= read -r line && ([[ -z $num_rows ]] || [[ $count -lt $num_rows ]]); do
	script_path="${script_output_dir}/b4_aviary_assemble_single_${line}.sh" 

#Put script below, using double backslash for continuing line
cat <<EOF > ${script_path}
#!/bin/bash

#SBATCH --job-name=aviary_assemble_single_mh${line}
...
aviary assemble \
-1 ${sequence_input_dir}/${line}.fastq \\
-o ${aviary_export_dir}/aviary_${line} \\
-m 256 \\
--kmer-sizes 27 37 47 57 67 77 87 97 107 117 127 \\
-n 24 \\
--use-megahit

conda deactivate

#Delete fastq files as they are completed:
#rm ${sequence_input_dir}/${line}_1.fastq
#rm ${sequence_input_dir}/${line}_2.fastq

#this is for Single read
#rm ${sequence_input_dir}/${line}.fastq
